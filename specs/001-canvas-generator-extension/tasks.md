...
- [X] T006 [P] Define strict snapshot, request with ordered `phaseOutputs`, phase-artifact binding, finalized-override, blueprint, result, and receipt schemas in `spec-kit-extensions/pipeline-canvas-generator/schemas/artifact-snapshot.schema.json`, `generation-request.schema.json`, `phase-artifact-binding.schema.json`, `command-override.schema.json`, `pipeline.schema.json`, `generation-result.schema.json`, and `generation-receipt.schema.json`; preserve `(kind, name)` identity and the fields in `contracts/composition-and-generation.md`.
...
- [X] T010 Implement shared Wizard/CLI request preparation in `spec-kit-extensions/pipeline-canvas-generator/scripts/lib/request.py`: preflight the tested Specify minimum, capture one immutable snapshot, preserve selected phase order, and derive stable canvas ID/target. Resolve one effective preset-replaceable `phase-outputs` Specify template and bind each selected phase's validated result, provider provenance, source path, hash, and full ordered template stack. When an entry is absent, bind a safe skill-derived path hint or unknown output without blocking a callable command. Reject ambiguous, unsafe, or malformed configuration and any `canvas-design` contributor to a selected runtime phase command; write no request on error. Keep Specify's resolution authority and do not treat hints as verified artifacts.
...
- [X] T034 [US2] Document installation, skills reload, CLI invocation, whole-document `phase-outputs` template replacement and incompatibility diagnostics, capability errors, and transfer without Wizard in `spec-kit-extensions/pipeline-canvas-generator/README.md`.
