// Shared viewer controls for selecting, applying, and retaining clarification drafts.
...
export function observationMessage(observed) {
    ...
    if (observed.timedOut) return "Timed out waiting for an artifact update. Drafts retained; the agent may still be editing. Review the artifact before retrying.";
    return "Submitted; waiting for an artifact update. Drafts remain editable.";
}

export function refreshDraftControls(root, view, queue, { apply }) {
    ...
    banner.innerHTML = `<span role="status">${esc(view.message || `${view.marks.length} open clarifications. ${applicable.length} draft${applicable.length === 1 ? "" : "s"} saved.`)}</span>
        ...
        <button class="btn btn-primary btn-sm" id="apply-clarifications" type="button" ${sending || !count ? "disabled" : ""}>${sending ? "Submitting…" : `Apply answers (${count})`}</button>` : ""}
    ...
    banner.querySelectorAll("[data-draft-select]").forEach((input) => input.addEventListener("change", () => {
        ...
        refreshDraftControls(root, view, queue, { apply });
