...
import { resolveWorkspaceDirectory } from "./runtime/workspace-files.mjs";
...
async function location(cwd, key, create = false) {
    for (const relative of [".speckit-canvas", ".speckit-canvas/artifact-reviews"]) {
    ...
    return join(await resolveWorkspaceDirectory(cwd, ".speckit-canvas/artifact-reviews"), `${key}.json`);
}
